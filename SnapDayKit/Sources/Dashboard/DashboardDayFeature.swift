...
@Reducer
public struct DashboardDayFeature: TodayProvidable {
  ...
  public struct State: Equatable, TodayProvidable {
    ...
    var dayInformation: InformationViewConfiguration? {
      ...
      if selectedDay.activities.allSatisfy(\.isDone) && !selectedDay.activities.isEmpty {
        ...
        return selectedDay.date < today ? .pastDay : .todayOrFuture
      }
      return nil
    }
  ...
  public enum Action: BindableAction, Equatable {
    public enum ViewAction: Equatable {
      ...
      case listItemActionPerformed(ListItemAction)
    ...
    public enum DelegateAction: Equatable {
      case calendarPickerRequested(CalendarPickerFeature.State)
      case premiumAccessRequested
      case reloadRequested
      case remindersReloadRequested
    }

    case activityList(PresentationAction<ActivityListFeature.Action>)
    case binding(BindingAction<State>)
    case dayActivity(DayActivityAction)
    case dayActivityTask(DayActivityTaskAction)
    case dayActivityTaskForm(PresentationAction<DayActivityFormFeature.Action>)
    case delegate(DelegateAction)
    case dismissPresentations
  ...
  public var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case .dayActivity(let action):
        return handleDayActivityAction(action, state: &state)
      case .dayActivityTask(let action):
  ...
  private func saveOrder(state: State) -> Effect<Action> {
    .run { [selectedDay = state.selectedDay] send in
      guard var selectedDay else { return }
      for index in selectedDay.activities.indices {
        selectedDay.activities[index].position = index
        try await dayUpdater.saveDayActivity(
          selectedDay.activities[index],
          syncSharable: false
        )
      }
  ...
  private func performListItemAction(
    _ actionType: ListItemAction,
    state: inout State
  ) -> Effect<Action> {
    switch actionType {
    case .itemTapped(let itemId, let parentId):
      return .run { send in
        if let parentId,
          ...
          await send(.dayActivityTask(.showEditForm(task)))
        } else if let dayActivity = try await dayUpdater.dayActivity(identifier: itemId) {
          await send(.dayActivity(.showEditForm(dayActivity)))
        }
      }
    case .rowAction(let parameters):
      guard let action = DayActivityInvitationAction(rawValue: parameters.actionId),
            let dayActivity = state.selectedDay?.activities.first(where: {
              $0.id.uuidString == parameters.itemId
            }) else {
        return .none
      }
      return .run { send in
        switch action {
        case .accept:
          await send(.dayActivity(.acceptInvitation(dayActivity)))
        case .discard:
          await send(.dayActivity(.discardInvitation(dayActivity)))
        }
      }
    case .menuAction(let menuParameters, let submenuParameters):
      return .run { send in
        if let parentId = menuParameters.parentId,
           let dayActivity = try await dayUpdater.dayActivity(identifier: parentId),
           let task = dayActivity.dayActivityTasks.first(where: {
             $0.id.uuidString == menuParameters.itemId
           }),
           let action = DayActivityTaskAction(rawValue: menuParameters.actionId) {
          switch action {
          case .deselect, .select:
            await send(.dayActivityTask(.select(task)))
          case .edit:
            await send(.dayActivityTask(.showEditForm(task)))
          case .remove:
            await send(.dayActivityTask(.remove(task)))
          }
        } else if let dayActivity = try await dayUpdater.dayActivity(
          identifier: menuParameters.itemId
        ), let action = DayActivityAction(rawValue: menuParameters.actionId) {
          switch action {
          case .deselect, .select:
            await send(.dayActivity(.select(dayActivity)))
          case .edit:
            await send(.dayActivity(.showEditForm(dayActivity)))
          case .addTask:
            await send(.dayActivityTask(.showNewForm(dayActivity)))
          case .save:
            await send(.dayActivity(.save(dayActivity)))
          case .move:
            await send(.dayActivity(.showDatePicker(dayActivity)))
          case .copy:
            await send(.dayActivity(.showMultiDatePicker(dayActivity)))
          case .remove:
            await send(.dayActivity(.remove(dayActivity)))
          case .selectImportant:
            await send(.dayActivity(.setImportant(true, dayActivity)))
          case .deselectImportant:
            await send(.dayActivity(.setImportant(false, dayActivity)))
          case .deselectCollaborator, .selectCollaborator:
            guard let submenuParameters,
                  let submenuAction = DayActivityCollaborationAction(
                    rawValue: submenuParameters.actionId
                  ) else { return }
            switch submenuAction {
            case .add:
              await send(.dayActivity(.addParticipant(submenuParameters.itemId, dayActivity)))
            case .remove:
              await send(.dayActivity(.removeParticipant(submenuParameters.itemId, dayActivity)))
            }
          case .stopCollaboration:
            await send(.dayActivity(.stopCollaboration(dayActivity)))
          }
        }
      }
      ...
      return .run { [activities = state.selectedDay?.activities] send in
        ...
        case .perform(let destinationId):
          guard let dayActivity = activities?.first(where: { $0.id.uuidString == itemId }),
                let destination = activities?.first(where: {
                  $0.id.uuidString == destinationId
                }) else { return }
          await send(.dayActivity(.reorder(dayActivity, destination)))
        case .drop:
          await send(.saveOrder)
        }
      }
    case .newItemForm(let action):
      return .send(.newItemForm(action))
    }
  }

  private func handleDayActivityAction(
    _ action: Action.DayActivityAction,
    state: inout State
  ) -> Effect<Action> {
    switch action {
    case .showNewForm:
    ...
    case .select(var dayActivity):
      ...
      return .run { [dayActivity] send in
        ...
        await send(.delegate(.reloadRequested))
        await send(.delegate(.remindersReloadRequested))
        guard dayActivity.hasIncompletedSubtasksAndDone else { return }
        await send(.dayActivity(.showAlertSelectAll(dayActivity)))
      }
    ...
    case .update(let dayActivity):
      ...
      let showAlertSelectAll = dayActivity.hasIncompletedSubtasksAndDone
        && activityBeforeUpdate?.isDone == false
      let showAlertSelectDayActivity = activityBeforeUpdate?.hasCompletedSubtasks == false
      ...
      return .run { [showAlertSelectAll, dayActivity] send in
        ...
        if showAlertSelectAll {
          ...
          await send(.dayActivity(.showAlertSelectActivity(dayActivity)))
        }
      }
    ...
    case .showDatePicker(let dayActivity):
      guard let selectedDay = state.selectedDay else { return .none }
      let picker = CalendarPickerFeature.State(
        type: .singleSelection(
          .navigationButton(title: String(localized: "Move", bundle: .module))
        ),
        date: selectedDay.date,
        objectIdentifier: dayActivity.id.uuidString,
        actionIdentifier: CalendarActivityAction.move.rawValue
      )
      return .send(.delegate(.calendarPickerRequested(picker)))
    ...
    case .reorder(let dayActivity, let destinationDayActivity):
      guard dayActivity.priority(calendar: calendar)
        == destinationDayActivity.priority(calendar: calendar),
    ...
    case .setImportant(let isImportant, let dayActivity):
      guard var dayActivity = findActivity(id: dayActivity.id, state: state) else {
        return .none
      }
  ...
  private func handleDayActivityTaskAction(
    _ action: Action.DayActivityTaskAction,
    state: inout State
  ) -> Effect<Action> {
    switch action {
    ...
    case .select(var dayActivityTask):
      ...
      return .run { [dayActivityTask] send in
        ...
        await send(.delegate(.reloadRequested))
        await send(.delegate(.remindersReloadRequested))
        guard let dayActivity = try await dayUpdater.dayActivity(
          identifier: dayActivityTask.dayActivityId.uuidString
        ), dayActivity.hasCompletedSubtasksAndNotDone else { return }
        await send(.dayActivity(.showAlertSelectActivity(dayActivity)))
      }
    case .create(let dayActivityTask):
      guard var dayActivity = state.selectedDay?.activities.first(where: {
        $0.id == dayActivityTask.dayActivityId
      }) else { return .none }
    ...
    case .update(let dayActivityTask):
      let dayActivityTaskBeforeUpdate = findActivityTask(
        id: dayActivityTask.id,
        activityId: dayActivityTask.dayActivityId,
        state: state
      )
      let wasCompleted = dayActivityTaskBeforeUpdate?.isDone == false
  ...
  private func handleNewItemFormAction(
    _ action: NewItemFormAction,
    state: inout State
  ) -> Effect<Action> {
    switch action {
    case .cancelled:
    ...
    case .submitted:
      guard let newField = state.newField,
        ...
        return .send(.newItemForm(.cancelled))
      }
      switch newField {
      case .activityName:
        ...
        state.newField = nil
        return .send(.dayActivity(.create(dayActivity)))
      case .taskName(let activityId):
        ...
        state.newField = nil
        return .send(.dayActivityTask(.create(dayActivityTask)))
      }
    }
  }
  ...
  private func handleDayActivityFormAction(
    _ action: PresentationAction<DayActivityFormFeature.Action>,
    state: inout State
  ) -> Effect<Action> {
    switch action {
    case .presented(.delegate(.activityUpdated(let form))):
      ...
      return .send(.dayActivity(.update(dayActivity)))
    case .presented(.delegate(.activityDeleted(let form))):
      ...
      return .send(.dayActivity(.remove(dayActivity)))
  ...
  private func handleActivityListAction(
    _ action: PresentationAction<ActivityListFeature.Action>,
    state: inout State
  ) -> Effect<Action> {
    switch action {
    case .presented(.delegate(.daysUpdated)):
      return .send(.delegate(.reloadRequested))
    case .presented(.delegate(.premiumAccessRequested)):
      return .send(.delegate(.premiumAccessRequested))
    default:
      return .none
  ...
  private func handleDayActivityTaskFormAction(
    _ action: PresentationAction<DayActivityFormFeature.Action>,
    state: inout State
  ) -> Effect<Action> {
    switch action {
    case .presented(.delegate(.activityUpdated(let form))):
      guard var dayActivityTask = findActivityTask(form: form, state: state) else {
        return .none
      }
      dayActivityTask.update(by: form)
      return .send(.dayActivityTask(.update(dayActivityTask)))
    case .presented(.delegate(.activityDeleted(let form))):
      guard let dayActivityTask = findActivityTask(form: form, state: state) else {
        return .none
      }
      return .send(.dayActivityTask(.remove(dayActivityTask)))
    case .presented(.delegate(.premiumAccessRequested)):
      return .send(.delegate(.premiumAccessRequested))
    default:
      return .none
  ...
  private func resumePremiumAction(state: State) -> Effect<Action> {
    var effects: [Effect<Action>] = []
    if state.editDayActivity != nil {
      effects.append(.send(.editDayActivity(.presented(.premiumAccessGranted))))
    }
    if state.dayActivityTaskForm != nil {
      effects.append(.send(.dayActivityTaskForm(.presented(.premiumAccessGranted))))
    }
    if state.activityList != nil {
      effects.append(.send(.activityList(.presented(.premiumAccessGranted))))
    }
  ...
  private func updatePresentedPremiumEntitlement(
    ...
    var effects: [Effect<Action>] = []
    if state.editDayActivity != nil {
      effects.append(.send(.editDayActivity(.presented(.premiumEntitlementUpdated(hasAccess)))))
    }
    if state.dayActivityTaskForm != nil {
      effects.append(
        .send(.dayActivityTaskForm(.presented(.premiumEntitlementUpdated(hasAccess))))
      )
    }
    if state.activityList != nil {
      effects.append(.send(.activityList(.presented(.premiumEntitlementUpdated(hasAccess)))))
    }
    return .merge(effects)
  }
  ...
  private func handleViewAction(_ action: Action.ViewAction, state: inout State) -> Effect<Action> {
    switch action {
    case .newButtonTapped:
      return .send(.dayActivity(.showNewForm))
    case .activityListButtonTapped:
      guard let selectedDay = state.selectedDay else { return .none }
      ...
      return .none
    case .listItemActionPerformed(let action):
      ...
      case .incompleteSubtasks(let dayActivity):
        return .run { send in
          for dayActivityTask in dayActivity.dayActivityTasks {
            ...
            await send(.dayActivityTask(.select(dayActivityTask)))
          }
        }
      case .completeActivity(let dayActivity):
        return .send(.dayActivity(.select(dayActivity)))
      }
    case .cancelAlertButtonTapped:
      state.alert = nil
      return .none
